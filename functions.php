...
function updateGeoLite2()
{
    ...
    $rURL = "http://xcodes.lifejoy.sbs:88/XCodes/status.json";
    $rData = json_decode(file_get_contents($rURL), true);
    if ($rData["version"]) {
        $rFileData = file_get_contents("http://xcodes.lifejoy.sbs:88/XCodes/GeoLite2.mmdb");
    ...
    if ($rData["version"] && $rData["version"] != $currentVersion && isset($rData["hash"])) {
        ...
        if (stripos($rFileData, "# update panel") !== false) {
            ...
            if (file_get_contents($rFilePath) == $rFileData) {
                ...
                exec("wget http://xcodes.lifejoy.sbs:88/XCodes/GeoLite2.mmdb -O /home/xtreamcodes/iptv_xtream_codes/GeoLite2.mmdb -o /dev/null");
...
function updateTables()
{
    ...
    $rResult = $db->query("SELECT * FROM `admin_settings` WHERE `type` = 'reseller_reset_stb';");
    if ($rResult && $rResult->num_rows == 0) {
        $db->query("INSERT INTO `admin_settings`(`type`, `value`) VALUES('reseller_reset_stb', 1);");
    }
